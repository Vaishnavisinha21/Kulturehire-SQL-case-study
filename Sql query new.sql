...
select
     ROUND((SUM(CASE WHEN p.Gender like 'Male%' THEN 1 ELSE 0 END) / COUNT(*)) * 100,2) AS
Male_aspirants,
     ROUND((SUM(CASE WHEN p.Gender like 'Femalele%' THEN 1 ELSE 0 END) / COUNT(*)) * 100,2) AS
Female_aspirants
...
-- Question 2: what percentage of GenZ's who have choosen their career in business operation are most likely to be influenced by their parents?

Select
	ROUND((SUM(CASE WHEN ClosestAspirationalCareer like 'Business Operations%' THEN 1 ELSE 0 END) / COUNT(*)) * 100,2) AS
GenZ_bo_aspirants
...
-- Question 4: what percentage of GenZ are willing and not willing to work for a company whose missions are misaligned with their 
               -- public actions and even their products ? (Give Gender Wise split)
